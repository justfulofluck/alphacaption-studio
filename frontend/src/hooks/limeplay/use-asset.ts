...
function createDefaultLoad({
  ...
  return async (source, loadOptions) => {
    ...
    if (typeof resolvedSource === "string") {
      const lower = resolvedSource.toLowerCase()
      if (lower.endsWith(".mp4") || lower.includes(".mp4?") || lower.includes("/video/")) {
        mimeType = "video/mp4"
    ...
    await player.load(resolvedSource, timeToLoad, mimeType)
  }
}
