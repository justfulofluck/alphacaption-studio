...
function TimelineSetup() {
  ...
  const onDurationChange = React.useCallback(() => {
    ...
    const playerDuration = isLive && player
  ...
  }, [events, isLive, mediaElement, player, store])

  const onBuffer = React.useCallback(() => {
    if (!mediaElement) return

    const totalBuffered: shaka.extern.BufferedRange[] = []
    
    if (player && !player.isBuffering()) {
      totalBuffered.push(...player.getBufferedInfo().total)
    } else {
    ...
    store.setState(({ timeline }) => {
      timeline.buffered = totalBuffered
    })
  }, [player, mediaElement, store])
